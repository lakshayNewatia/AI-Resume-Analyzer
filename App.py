...
def run():
    ...
    if choice == 'User':
        ...
        if pdf_file is not None:
            ...
            # 2. Create a dictionary to hold the data (mimics the old library)
            resume_data = {
                "name": "User",
                "email": None,
                "mobile_number": None,
            ...
            # 3. REFINED NAME LOGIC (No more "Pandas")
            lines = [line.strip() for line in resume_text.split('\n') if line.strip()]
            blacklist = {'Pandas', 'Numpy', 'Spacy', 'Java', 'React', 'Python', 'Resume', 'CV', 'Page'}
            extracted_name = None

            if lines:
                for line in lines[:3]: # Check top 2 lines first
                    line_doc = nlp(line)
            ...
            if not extracted_name or extracted_name.lower() in [b.lower() for b in blacklist]:
            # Use Filename if AI fails, but clean it properly
            ...
            resume_data["name"] = extracted_name 
            # 4. Extract Email using Regex
            ...
            # 5. Extract Skills (matches your keyword lists against the text)
            # This combines all your keyword lists from your 'Courses.py' imports
